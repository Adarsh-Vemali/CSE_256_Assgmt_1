...
def read_word_embeddings_random(embeddings_file: str) -> WordEmbeddings:
    """
    Loads the given embeddings (ASCII-formatted) into a WordEmbeddings object. Augments this with an UNK embedding
    that is the 0 vector. Reads in all embeddings with no filtering -- you should only use this for relativized
    word embedding files.
    ...
    for line in f:
        if line.strip() != "":
            ...
            numbers = line[space_idx+1:]
            float_numbers = [float(number_str) for number_str in numbers.split()]
            vector = np.array([np.random.uniform(-0.01, 0.01) for _ in float_numbers]) # random initialisations
            # vector = np.array(float_numbers)
            word_indexer.add_and_get_index(word)
            # Append the PAD and UNK vectors to start. Have to do this weirdly because we need to read the first line
            # of the file to see what the embedding dim is
...
def read_word_embeddings_random_BPE(vocab: str):
    ...
    for subword in vocab:
        word_indexer.add_and_get_index(subword)
        vector = np.random.uniform(-0.01, 0.01, size=300) # hardcoded
        # Append the PAD and UNK vectors to start. Have to do this weirdly because we need to read the first line
        # of the file to see what the embedding dim is
        if len(vectors) == 0:
            vectors.append(np.zeros(vector.shape[0]))
